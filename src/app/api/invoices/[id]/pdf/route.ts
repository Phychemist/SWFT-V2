...
// GET - Generate 60-minute signed URL and redirect
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // 2. Generate a 60-minute signed URL for the pdf file
    const filePath = `${invoice.uid}.pdf`
    ...
    console.error('Error redirecting to invoice PDF:', error)
    return NextResponse.json<ApiResponse<null>>(
      { success: false, error: error.message || 'Failed to download invoice PDF' },
      { status: 500 }
    )
  }
}
